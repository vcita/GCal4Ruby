...
module GCal4Ruby
  ...
  class Service
    ...
    def free_busy_feed(id, from, to)
      return create_url("www.google.com/calendar/feeds/default/freebusy/busy-times/#{id}?start-min=#{from}&start-max=#{to}")
    end
    ...
    def freebusy(id, start, end_time)
      parameters =  {
          'max-results' => '2000',
          'sortorder' => 'ascending',
          'orderby' => 'starttime'}

      ret = send_request(GData4Ruby::Request.new(:get, free_busy_feed(id, Time.parse((start - 1.day).to_s).utc.xmlschema, Time.parse((end_time + 1.day).to_s).utc.xmlschema), nil,  parameters))
      freebusy = []
      REXML::Document.new(ret.body).root.elements.each("gCal:busy"){}.map do |entry|
        entry = GData4Ruby::Utils.add_namespaces(entry)
        event = Freebusy.new(self, {:debug => debug})
        event.load(entry.to_s)
        freebusy << event
      end
      return freebusy
